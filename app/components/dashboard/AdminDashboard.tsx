   ...
   const handleTeamAssignment = async (userId:string, teamId:string | null)=>{
      startTransition(async()=>{
          try{
            ...
            ? error.message
            :"Error updating team asssignment"
            );
        }
      });
   };

   const  handleRoleAssignment = async(userId:string,newRole:Role |null)=>{
    ...
    return <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
            {/*Users table with role and team assignment */}
               <div className="bg-slate-800 border border-salte-700 rounded-lg">
                                        ...
                                        <select
                                         ...
                                         value={user.teamId || " "}
                                          onChange={(e)=>
                                           ...
                                           <td className=" py-2">
                                               {user.teamId && (
                                                <button onClick={()=> handleTeamAssignment(user.id, null)}
                                                disabled ={isPending}
                                                className="text-red-400 hover:text-red-300 text-xs disabled:opactity-50"
                                                >
                                                  Remove
                                                </button>
                                               )}
            ...
            {/*Teams table */}
             <div className="bg-slate-800 border border-salte-700 rounded-lg">
               ...
               {/* Stats */}
               <div className="grid grid-cols-1 md:grid-clos-5 gap-4 ">
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 text-center">
                    <div className="text-2xl font-bold text-white">{users.length}</div>
                   <div className="text-sm text-slate-400">Total users</div>
                </div>

                <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 trxt-center">
